...
//        ImageUtils.pressBackGroudText("1","C:\\Users\\outsider\\Desktop\\123.jpg","C:\\Users\\outsider\\Desktop\\new.jpg","宋体",Font.BOLD,10, 200, 200, Color.white,Color.black);


//        byte[] data=Files.readAllBytes(Paths.get("map/123.jpg"));
//        byte[] bytes=Base64.getEncoder().encode(data);

//        InputStream inputStream=new ByteArrayInputStream(data);
//        System.out.println(ImageUtils.pressBackGroudText("1",inputStream,"宋体",Font.BOLD,20, 200, 200, Color.white,Color.black));

//        Random random=new Random();
//        Map<String,Point> map=new HashMap<>();
//        for (int i=1;i<=100;i++){
//            map.put(String.valueOf(i),new Point(random.nextInt(1200),random.nextInt(620)));
//        }
//        byte[] data=Files.readAllBytes(Paths.get("map/111.jpg"));
//        byte[] bytes=Base64.getEncoder().encode(data);
//        long a=System.currentTimeMillis();
//        System.out.println(ImageUtils.pressBatchBackGroudText(map,new String(bytes),"宋体",Font.BOLD,20,  Color.white,Color.black));
//        System.out.println(System.currentTimeMillis()-a);
    ...
    public final static String pressBatchBackGroudText(Map<Integer,Point> pressTextMap, ListMultimap<Integer, Point> pointMap, String imgBase64,
                                                       String fontName,
                                                       int fontStyle, int fontSize, Color fontColor, Map<Integer,Color> backGroundColorMap){
        try {
            byte[] bytes = new BASE64Decoder().decodeBuffer(imgBase64.trim());
            ...
            for (Map.Entry<Integer,Point> entry:pressTextMap.entrySet()){
...
//                int length = Math.max(fontHight, fontWidth);
//                g.fillArc((int)entry.getValue().getX()-length/4,(int)entry.getValue().getY()-length/4*3,length,length, 0, 360);

                java.util.List<Point> points = pointMap.get(entry.getKey());
                for(int i = 0; i < points.size() - 1; i++) {
                    g.drawLine((int) points.get(i).getLng(), (int) points.get(i).getLat(), (int) points.get(i + 1).getLng(), (int) points.get(i + 1).getLat());
                }
    ...
    public final static String pressBackGroudText(String pressText,InputStream inputStream,
                                         String fontName,
                                       int fontStyle, int fontSize,int x,
                                       int y, Color fontColor,Color backGroundColor) {
        try {
            Image src = ImageIO.read(inputStream);
            int width = src.getWidth(null);
            int height = src.getHeight(null);
            ...
            g.fillRect(x,y+1-fontHight,fontWidth,fontHight);
